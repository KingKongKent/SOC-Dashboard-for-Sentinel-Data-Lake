...
if [ ! -f "${CONF_DIR}/.env" ]; then
    ...
    # Strip Windows \r from .env.example when copying
    sed 's/\r$//' "${APP_DIR}/.env.example" > "${CONF_DIR}/.env"
else
...
for KEY in CLIENT_ID CLIENT_SECRET TENANT_ID SECRET_KEY REDIRECT_URI CORS_ORIGINS ADMIN_USERS; do
    VAL=$(grep -E "^${KEY}=" "${CONF_DIR}/.env" | cut -d'=' -f2- | tr -d '\r')
...
cd "${APP_DIR}"
# Ensure DB dir contents are writable by service user (fixes re-deploy where root created files)
chown -R "${APP_USER}:${APP_USER}" "${DB_DIR}"
sudo -u "${APP_USER}" env DB_PATH="${DB_DIR}/soc_dashboard.db" "${VENV_DIR}/bin/python" database.py
...
SERVER_NAME="_"
if [ -f "${CONF_DIR}/.env" ]; then
    REDIR=$(grep -E "^REDIRECT_URI=" "${CONF_DIR}/.env" | cut -d'=' -f2- | tr -d '\r' | sed 's|https\?://||;s|/.*||')
...
# ── 8b. Detect upstream proxy_protocol ──────────
# If DNS points somewhere other than this LXC, traffic likely arrives
# via an SNI proxy that adds proxy_protocol.  Auto-detect via .env
# or fall back to checking the default gateway's subnet.
PROXY_IP=""
if [ -f "${CONF_DIR}/.env" ]; then
    PROXY_IP=$(grep -E "^UPSTREAM_PROXY_IP=" "${CONF_DIR}/.env" | cut -d'=' -f2- | tr -d '\r')
fi
USE_PP=""
if [ -n "$PROXY_IP" ]; then
    USE_PP="yes"
    echo "  → proxy_protocol enabled (upstream proxy: ${PROXY_IP})"
else
    echo "  → proxy_protocol disabled (no UPSTREAM_PROXY_IP in .env)"
    echo "    Set UPSTREAM_PROXY_IP=<ip> in .env if behind an SNI proxy"
fi

# Helper: listen directive with or without proxy_protocol
if [ -n "$USE_PP" ]; then
    LISTEN_443="listen 443 ssl http2 proxy_protocol"
    LISTEN_443_V6="listen [::]:443 ssl http2 proxy_protocol"
    REAL_IP_BLOCK="    # Extract real client IP from proxy protocol
...
# ── 8c. Self-signed TLS (if no Let's Encrypt cert) ─
...
# Self-signed TLS — replace with Let's Encrypt for production:
#   certbot certonly --webroot -w /var/www/html -d ${SERVER_NAME}
# WARNING: Do NOT use "certbot --nginx" — it rewrites listen directives
#   and removes proxy_protocol, breaking TLS behind an upstream SNI proxy.
...
SSLEOF
    ...
    # Let's Encrypt cert exists — only install template if no HTTPS config exists.
    # Never overwrite an existing config that already has listen 443 ssl +
    # proxy_protocol — doing so will break TLS behind an upstream SNI proxy.
    NGINX_CONF="/etc/nginx/sites-available/soc-dashboard"
    if [ -f "${NGINX_CONF}" ] && grep -q "listen.*443.*ssl" "${NGINX_CONF}" 2>/dev/null; then
        echo "  ✅ Let's Encrypt cert found — existing HTTPS config preserved"
    else
        echo "  ✅ Let's Encrypt cert found — installing template config"
        cp "${APP_DIR}/scripts/nginx_site.conf" "${NGINX_CONF}"
        sed -i "s/YOUR_DOMAIN/${SERVER_NAME}/g" "${NGINX_CONF}"
        if [ -n "$PROXY_IP" ]; then
            sed -i "s/PROXY_IP/${PROXY_IP}/g" "${NGINX_CONF}"
        else
            # No proxy — strip proxy_protocol lines from template
            sed -i 's/ proxy_protocol//g' "${NGINX_CONF}"
            sed -i '/set_real_ip_from/d; /real_ip_header proxy_protocol/d' "${NGINX_CONF}"
        fi
    fi
fi
...
if nginx -t 2>&1; then
    ...
    echo "  ❌ nginx config test FAILED — check /etc/nginx/sites-available/soc-dashboard"
    echo "  → Run: nginx -t  to see the error details"
fi
...
# Validate nginx config (service can be 'active' with a stale config after failed reload)
if nginx -t 2>/dev/null; then
    echo "  ✅ nginx config: valid"
else
    echo "  ❌ nginx config: INVALID (run 'nginx -t' for details)"
    HEALTH_OK=false
fi
...
echo ""
echo "  For production TLS: certbot certonly --webroot -w /var/www/html -d <YOUR_DOMAIN>"
echo "============================================="

...
# Using single-quoted here-string to prevent PowerShell expansion.
$remoteScript = @'
#!/usr/bin/env bash
# =============================================================
...
if [ -n "$LOG_DIR" ] && [ -f "$LOG_DIR/error.log" ]; then
    URI_ERRORS=$(grep -c "no URI read\|Invalid HTTP_HOST\|PROXY.*Invalid" "$LOG_DIR/error.log" 2>/dev/null | tail -1)
    URI_ERRORS=${URI_ERRORS:-0}
fi
JOURNAL_URI=$(journalctl -u dashboard.service --no-pager -q 2>/dev/null | grep -c "no URI read" | tail -1)
JOURNAL_URI=${JOURNAL_URI:-0}
...
foreach ($target in $Hosts) {
    ...
    # Write script to temp file with LF line endings, pipe to SSH
    $tmpScript = [System.IO.Path]::GetTempFileName()
    [System.IO.File]::WriteAllText($tmpScript, $finalScript, [System.Text.UTF8Encoding]::new($false))
    try {
        Get-Content $tmpScript -Raw | ssh -p $SshPort -o ConnectTimeout=10 -o StrictHostKeyChecking=accept-new $target 'bash -s' 2>&1 | Tee-Object -FilePath $outFile

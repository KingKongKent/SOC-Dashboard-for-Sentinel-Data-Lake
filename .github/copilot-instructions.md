...
### Deployment / LXC
- **Proxy protocol required**: CT 204 (192.168.7.20) nginx expects a PROXY protocol preamble from upstream. Direct connections (bypassing the proxy at 192.168.7.10) cause `ERR_CONNECTION_RESET`.
- **DNS must point to proxy**: Pi-hole DNS for `report.kents-events.com` must resolve to `192.168.7.10` (www-proxy), **NOT** `192.168.7.20` (the LXC directly).
...
- **certbot ACME challenge**: Needs HTTP :80 traffic for `report.kents-events.com` forwarded from the proxy to CT 204. Without this, certificate renewal fails silently.
